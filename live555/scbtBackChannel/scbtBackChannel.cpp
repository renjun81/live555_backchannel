...
char* WAVE_FILE = "beep.wav";

void startPlay(RTSPClient* rtspClient);
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString) {
    do {
        ...
        if (resultCode != 0) {
            env << *rtspClient << " code : " << resultCode
...
: iter(NULL), session(NULL), subsession(NULL), streamTimerTask(NULL), duration(0.0), eventLoopWatchVariable(0) {
}
...
Boolean MediaSubsession::createSinkObjects(int useSpecialRTPoffset)
{
    do {
        ...
        WAVAudioFileSource* waveSource =  WAVAudioFileSource::createNew(fParent.envir(), WAVE_FILE);
        fReadSource = uLawFromPCMAudioSource::createNew(fParent.envir(), waveSource);

        //fReadSource = WAVAudioFileSource::createNew(fParent.envir(), "beep.wav");
