...
void setupNextSubsession(RTSPClient* rtspClient) {
    ...
    if (scs.subsession != NULL) {
        if ( strcmp(scs.subsession->mediumName(),"audio") != 0
        		//(scs.subsession->getFlag() == 0 && bEnableBackChannel)
...
::createNewStreamSource(unsigned /*clientSessionId*/, unsigned& estBitrate) {
    ...
    FramedSource *pTmp = ADTSAudioFileSource::createNew(fParent.envir(), "send.aac");
#endif
    return pTmp;
}
...
Boolean MediaSubsession::createSinkObjects(int useSpecialRTPoffset)
{
    do {
        ...
        fReadSource =  WAVAudioFileSource::createNew(fParent.envir(), gsz_backSound);
        if ( bPassMuFilter )
        	fReadSource = uLawFromPCMAudioSource::createNew(fParent.envir(), fReadSource);
        ...
        fRTPSink = SimpleRTPSink::createNew(fParent.envir(), fRTPSocket,
        		fRTPPayloadFormat, 8000, "audio", "PCMU", 1, false);
